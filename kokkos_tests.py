...
def main():
    ...
    parser.add_argument("operations", choices=list(operations_dict.keys()), nargs="+", help="Operation(s) to perform")
    ...
    parser.add_argument("-n","--trials", type=int, default=1, help="Number of trials to perform per operation")
    parser.add_argument("--timesout", help="Path to output csv of execution times")
    ...
    exec_times = []
    operations = {operation:operations_dict[operation] for operation in args.operations}

    contains_matrix_operation = False
    contains_vector_operation = False
    for operation_func, operand_types in operations.values():
        if 'm' in operand_types:
            contains_matrix_operation = True
        if 'v' in operand_types:
            contains_vector_operation = True    
    ...
    if contains_matrix_operation and contains_vector_operation:
    ...
    index_instance = 0
    num_instances = len(operations)
    for operation, operation_info in operations.items():
        operation_func, operand_types = operation_info
        operands = []
        ...
        print("Starting excecution of", operation)
        for iteration in range(1,args.trials+1):
            print(f"Iteration {iteration}/{args.trials} of {operation}")
            timer2 = default_timer()
            print(operation,operation_func,operands)
            out_tensor = operation_func(*operands, index_instance=index_instance, num_instances=num_instances, options=options)
            elapsed = default_timer() - timer2
            exec_times.append([operation,iteration,elapsed])
            print(f"Time elapsed for execution (my include compilation): {elapsed:.4f} seconds")
        if args.verbose:
    ...
    if args.timesout:
        with open(args.timesout, 'w', newline='') as csvfile:
            timeswriter = csv.writer(csvfile)
            timeswriter.writerow(["Operation","Iteration","Execution Time in seconds"])  # header
            for row in exec_times:
                timeswriter.writerow(row)
        print("Finished Writing execution times to", args.timesout)
    print("Done.")
